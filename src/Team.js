				...
				<ul className="space-y-12 lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8 lg:gap-y-12 lg:space-y-0">
					...
					<li key={person.name}>
						<div className="space-y-4 sm:grid sm:grid-cols-3 sm:gap-6 sm:space-y-0 lg:gap-8">
							<div className="h-0 aspect-w-3 aspect-h-3 sm:aspect-w-4 sm:aspect-h-4">
								...
								<div className="space-y-4">
									<div className="text-lg leading-6 font-medium space-y-1">
										<div className="flex items-center">
											<h3>{person.name}</h3>
											{person.twitterUrl && <a href={person.twitterUrl} className="hover:text-gray-600 ml-3" onClick={() => (mixpanel.track('Navigation', { src: 'body', dest: `${person.name} twitter` }))} target="_blank" rel="noreferrer">
